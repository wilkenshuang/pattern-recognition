...
def dropfall(image):
    ...
    hist_width=_projection(skele_extrac)
...
def detect_connection(image):
    ...
    for i in range(skele_extrac.shape[1]):
        p_top=0
        for j in range(skele_extrac.shape[0]):        
            if skele_extrac[j,i]==0:
                p_top=max(j,p_top)
        points.append(p_top)
    i=1
    crest=[]
...
#    else:
    ...
    while 1<=i<len(points)-1:
        ...
        if 0.9*len(points)<=i<len(points)-1:
            length=0
            while points[i-1]<=points[i] and i<len(points)-1:
                i+=1
                length+=1
...
def mark_main(img_path):
    ...
    for i in range(len(res)):
        ...
        #ret,th=cv2.threshold(num,230,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
            ...
            single.append(img)
            '''
            #print(detect_connection(img))
            if detect_connection(img)>=2 :
                try:
                    continue
                    mark+=color_connection(img)
                except:
                    single.append(img)
            else:
                single.append(img) 
            '''                    
    mark+=color_single(single)
...
#img_path='C:/Users/gd/Desktop/test/543.jpg' 
start=time.clock()
print(mark_main(img_path))
end=time.clock()
print(end-start)
#print(mark_main(img_path))
